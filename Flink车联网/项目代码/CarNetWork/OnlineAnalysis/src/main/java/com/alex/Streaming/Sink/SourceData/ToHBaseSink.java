package com.alex.Streaming.Sink.SourceData;
 ...
 * @author Alex_liu
 * @Create 2022-11-21
 * @Description   将正常的数据实时的写入到hbase中
 *                实现思路：
 ...
 */
public class ToHBaseSink extends RichSinkFunction<VehicleDataObj> {
    //定义日志操作对象
    private final static Logger logger = LoggerFactory.getLogger(ToHBaseSink.class);
    //定义操作的hbase的表名
     ...
     * @param tableName
     */
    public ToHBaseSink(String tableName) {
    ...
    /**
     * 重写invoke方法：数据写入hbase逻辑，每条数据执行一次该方法
     * 设计hbase的rowkey、根据列族封装Put对象
     *    为了避免热点问题：原始数据实时ETL数据存储hbase使用vin+终端时间反转
     *    把封装好的Put对象，写入HBase表中，每条数据执行一次该方法
     * @param value
     * @param context
     * @throws Exception
     */
     ...
     * @return
     */
    public Put setDataSourcePut(VehicleDataObj vehicleDataObj) {
        //如何设计rowkey
        //使用vehicleDataObj.getVin() 会破坏了唯一原则
        //vehicleDataObj.getVin() + vehicleDataObj.getTerminalTime()：热点问题
        //需要将时间戳数据反转（自己实现）
        String rowKey = vehicleDataObj.getVin() + "_" + StringUtil.reverse(vehicleDataObj.getTerminalTimeStamp().toString());
        //定义列族的名称
        String cf = "cf";
    ...
    /**
     * 重写close方法：销毁对象，释放资源，遵循“先创建后关闭“原则
     * 释放资源的时候执行
     * @throws Exception
     */
    @Override
