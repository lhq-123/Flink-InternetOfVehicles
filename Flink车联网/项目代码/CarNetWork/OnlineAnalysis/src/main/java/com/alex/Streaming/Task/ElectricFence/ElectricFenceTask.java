...
public class ElectricFenceTask extends BaseTask {
    public static void main(String[] args) throws Exception{
        ...
        DataStream electricFenceVehicleInfoStream = env.addSource(new MysqlElectricFenceVehicleInfoSource()).broadcast();
        //TODO 6）90秒翻滚窗口电子围栏对象模型流数据与电子围栏分析结果数据广播流进行connect
        ConnectedStreams connectedStreams = electricFenceDataStream.connect(electricFenceVehicleInfoStream);
        ...
        mysqlStream.addSink(new ElectricFenceToMysql());
        env.execute();
    }
}
