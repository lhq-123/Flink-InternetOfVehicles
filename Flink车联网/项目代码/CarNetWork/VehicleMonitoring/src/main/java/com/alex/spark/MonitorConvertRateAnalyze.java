...
//			SparkUtils.mockData(sc, spark);
        ...
        // 3、查询任务，获取任务的参数
        long taskId = ParamUtil.getTaskIdFromArgs(args, Constants.SPARK_LOCAL_TASKID_MONITOR_ONE_STEP_CONVERT);
         ...
         */
        String roadFlow = ParamUtil.getParam(taskParam, Constants.PARAM_MONITOR_FLOW);
        final Broadcast<String> roadFlowBroadcast = sc.broadcast(roadFlow);

        /**
         * 通过时间的范围拿到合法的车辆
         */
        JavaRDD<Row> rowRDDByDateRange = SparkUtil.getCameraRDDByDateRange(spark, taskParam);
        /**
        ...
        for (int i = 0; i < split.length; i++) {
            ...
            if(count != 0L){
                ...
                if(i != 0 && lastMonitorCarCount != 0L){
                    double rate = NumberUtil.formatDouble((double)count/(double)lastMonitorCarCount,2);
            ...
            public Iterator<Tuple2<String, Long>> call(Tuple2<String, Iterable<Row>> tuple) throws Exception {
                ...
                Collections.sort(rows, new Comparator<Row>() {
                    ...
                    public int compare(Row row1, Row row2) {
                        ...
                        if(DateUtil.after(actionTime1, actionTime2)){
                            return 1;
                        }else {
                            return -1;
                        }
